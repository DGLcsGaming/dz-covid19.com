  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV === "production") {
      /*       Axios.get("https://algeriacovid19.herokuapp.com/api/which_server", {
        headers: { "x-access-token": process.env.REACT_APP_API_KEY, mode: "cors" },
      })
        .then((data) => data.data.data.server)
        .then((server) => {
          if (server === 1) { */
      socket = openSocket("https://algeriacovid19.herokuapp.com", {
        path: "/ws",
        ransports: ["websocket"],
      });
      setCurrentServer(1);
      ...
      socket = openSocket("localhost:4000", { path: "/ws", ransports: ["websocket"] });
      setCurrentServer(1);
    }
    return () => {
